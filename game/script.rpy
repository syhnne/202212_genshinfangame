...
label before_start:
    if persistent.playthrough == 1:
        '开始之前，先解释一下有关这个游戏的大概玩法以及剧情的事情'

        '首先，这个游戏的核心是一个地图，依靠玩家每天在地图上选择地点来触发剧情，'
        '根据触发剧情的不同，最后会导向不同的结局。'
        '正因如此，不同的选项有很多种，特殊事件也有很多，所以这玩意实际上是工程量很大的（'

        '其次，更加大它工程量的是，这游戏有2周目和3周目，有至少3种不同的结局。'
        '虽然23周目和1周目玩法完全一样，但是剧情会发生一些变化，大多数剧情是需要修改一部分台词，有个别剧情需要完全重写。'
        '这是因为我用了那种很烂大街的梦结局，是我为了把温馨小日常、寿命论刀子以及一些其他的要素全部塞进同一个游戏里而出的下策（。'

        '然后，我刚开始做这东西的时候，去玩了一下ddlc，感觉受到了很大启发，所以我会在23周目加一些奇怪的东西（'
        '当然，请放心，这不包括任何血腥猎奇画面。我指的是一些比较特别的，，只有游戏才能做到的表现手法……'
        '举个例子，就是须弥主线花神诞祭那段选糖果盒子，那个画面属实有点恐怖了，给我印象很深刻，就是这个：'
        menu:
            '没问题，这五个糖果盒子，从左到右是一号到五号，你选几号呢？'
            '四号。':
                pass
            '四号。':
                pass
            ...
            '四号。':
                pass
        '就是这段。'
        '不过这个恐怖程度算是很低的了，我加了一些别的，如果是第一次看的话，确实会被吓，我自己就被吓了（。说明效果是很好的'
        '具体是什么，玩到二周目就知道了。'
        '当然，那部分我还没做完，以后大概率会加大一些力度，这取决于23周目的剧情具体怎么安排。'

        '最后是我实际上对这剧本没啥自信，总觉得我能想到的剧情已经被别人写了八百回了，'
        '所以除了核心玩法（指地图）之外，其他任何看不顺眼的地方都可以改，加剧情也是可以随便加的，只要写得完（'
        '鉴于玩游戏的过程中不一定能刷出所有剧情，代码也没有那么简单易读，'
        '我准备了一个简单的{i}{a=https://docs.qq.com/sheet/DVkZDdmJmalZtaE95?tab=vnr83w}excel表格（这是个链接，可以点）{/i}{/a}，上面记录了剧情与它对应的出现时间，这样比较方便阅读，但是没有对具体剧情做太多说明，因为我自己也没太想明白（'
        '凡是游戏与表格不一致的地方，都以表格为准，游戏里的一律视为bug。。虽然我捋了几遍游戏了，但难免还是有bug，回头等快完工了，我会薅一群人来给我测试游戏的（'
        '关于这个time，现在屏幕顶上应当有一行小字‘t0 f0’，那个t后面的数字就是time，f是好感度。'
        '另外，如果要查看剧情代码，也挺简单的：游戏压缩包中有一个game文件夹，点开后往下翻找到【script.rpy】用记事本打开，就能看到所有剧情。'
        '（不过，建议还是装个文本编辑器，看着比记事本方便一点（'
        '另外，不要在意地图上最右边的‘好感+100’那栏。那是开发者模式下专门用来跳结局的，玩家看不到（'
    elif persistent.playthrough == 2:
        '这应该是二周目了罢，你可以看到首页发生了一些小小的变化，这个我还在改，以后变化会更大。'
...
label start_c:
    ...
    scene bg lyg 1
    show childe 1111 at tc
    '这是我之前拿blender渲的立绘，但是现在我改主意了，我感觉这个画风和背景格格不入'
    '反正我lora模型都炼好了，肯定要上ai辅助，但至于我是直接放ai图还是对它进行一个描改，要视情况而定'
    c 1234 '(这是台词)'
    '另外，目前的blender版立绘一共有7*7*7种表情，因为所有的眉毛眼睛嘴是可以排列组合的。'
    '你可以切成英文输入法，按两下pageup然后再按一下d，会看到台词前有一行数字，那就是用来改变表情的代码。'
