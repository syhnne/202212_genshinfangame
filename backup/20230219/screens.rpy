...
image cg 01 = 'cg 01.png'
image cg 02 = 'cg 02.png'
image cg 03 = 'cg 03.png'
image cg 04 = 'cg 04.png'
...
init python:
    ...
    gallery.transition = Dissolve(0.5, alpha = True)
    ...
    def gallery_add_image(image_list):
        ...
        for i in image_list:
            ...
            # 为了让我的电脑正确认识到他已经显示过这张图片了，后面image list当中所有的文件名必须先用image语句定义，然后写定义过的那个名字，不带引号的那种；但是如果这么写，上面那个第三句就会报错。
            # 一会再研究研究他用的这个collections到底是什么玩意儿。
            # 不，不是这个问题，我自欺欺人地把文件名和image语句声明改成一样的字，企图骗过renpy，没想到成功了？？
    ...
    def gallery_get_size():
        ...
        # 缩略图宽、高、偏移、间隔
        thumb_width = 1920 * gallery_config["thumb_scale"]
        thumb_height = 1080 * gallery_config["thumb_scale"]
    ...
    ('cg 01', _("描述1    空格测试") ),
    ('cg 02', _("描述2\n换行测试") ),
    ('cg 03', _("描述3 过多文字测试过多文字测试过多文字测试过多文字测试过多文字测试过多文字测试过多文字测试") ) ,
    ('cg 04', _("描述") ),
    ])
...
screen thegallery():
    ...
    use game_menu(_('画廊')):
        ...
        viewport:
            ...
            # 显示所有按钮区缩略图
            $ gallery_show_thumb()
        vbar value YScrollValue("gallery") xpos -30

        # 大图区，实际是画廊按钮，点击后全屏
        if now_image != None:
            $ pre_image = im.FactorScale(now_image+".png", 0.5)
...
screen quick_menu():
    ...
    if quick_menu:
        window:
            background 'gui/overlay/quickmenu.png'
            vbox:
                style_prefix "quick"
                ypos 5
...
screen preferences():
    ...
    use game_menu(_("设置"), scroll="viewport"):
        ...
        vbox:
            ...
            vbox:
                label '重置游戏数据'
                textbutton '点击重置' action Confirm('是否重置全部游戏数据？', Function(reset_game_data)) 
...
screen help():
    ...
    use game_menu(_("帮助"), scroll="viewport"):
        ...
        vbox:
            ...
            label '快进'
            text '按住ctrl以快进。'
            text '点击快捷菜单中的“快进”或按下Tab以启动快进，再次点击来取消。'
            text '右键点击快捷菜单中的“快进”可直接快进到下个选项。'
            label '保存及读取'
